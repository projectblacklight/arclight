...
RSpec.describe 'Search results', type: :feature do
  describe 'search results' do
    ...
    it 'renders the expected metadata for a collection', js: true do
      visit search_catalog_path q: 'omega', search_field: 'all_fields'

      expect(page).to have_css('.al-document-title-bar', text: 'National Library of Medicine. History of Medicine Division: MS C 271')
    ...
    it 'does not double escape entities in the heading' do
      visit search_catalog_path q: 'brief', search_field: 'all_fields'
      expect(page).to have_css(
        'h3.index_title',
